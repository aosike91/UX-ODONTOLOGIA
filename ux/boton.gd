...
@export var factor_escala: float = 1.05
@export var velocidad_transicion: float = 0.2
...
func activar_hover():
	...
	# Animar escala
	animate_scale(escala_original * factor_escala)
...
func desactivar_hover():
	...
	# Animar escala de vuelta
	animate_scale(escala_original)
...
func force_disable_hover():
	...
	mesh_instance.scale = escala_original
...
func animate_scale(target_scale: Vector3):
	...
	tween_escala.set_ease(Tween.EASE_OUT)
	tween_escala.set_trans(Tween.TRANS_CUBIC)
	tween_escala.tween_property(mesh_instance, "scale", target_scale, velocidad_transicion)
